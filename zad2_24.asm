...
str_maxstar db 4 dup (20h) , "$" ; string maximum stars
...
;str_maxstar db 3 dup (20h) , "$" ; string maximum stars
    ...
    mov cx , 3
    clear_buff:
    mov str_counter[bx] , 20h
    inc bx
    loop clear_buff
    
    mov cl , 10
    mov ax , word ptr star_count
    ...
    ; convert to string max value of stars
    mov cl , 10
    mov ax , stars
    mov bx , 02h ; writing reminders backwards (starting from position 2 , then 1 ..)
    max2str:
    ...
    mov ah , 02h ; set cursor to last row
    mov bh , 00h
    mov dh , 24
    mov dl , 00h
    int 10h
    
    mov ah , 09h  ; display how many stars left
    lea dx , str_counter
    int 21h
